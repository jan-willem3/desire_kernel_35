/*
 *  drivers/cpufreq/cpufreq_kenobi.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_KENOBI
static
#endif
struct cpufreq_governor cpufreq_gov_kenobi = {
    .name                   = "kenobi",
...
static struct workqueue_struct	*kkenobi_wq;
...
static void kenobi_early_suspend(struct early_suspend *handler)
{
    suspended = true;
...
static void kenobi_late_resume(struct early_suspend *handler)
{
    suspended = false;
...
static struct early_suspend kenobi_suspend = {
	.suspend = kenobi_early_suspend,
	.resume = kenobi_late_resume,
...
static void kenobi_powersave_bias_init_cpu(int cpu)
{
...
static void kenobi_powersave_bias_init(void)
{
    int i;
    for_each_online_cpu(i) {
	kenobi_powersave_bias_init_cpu(i);
    }
}
...
{
    printk_once(KERN_INFO "CPUFREQ: kenobi sampling_rate_max "
...
/* cpufreq_kenobi Governor Tunables */
...
#define DEPRECATION_MSG(file_name)				\
    printk_once(KERN_INFO "CPUFREQ: Per core kenobi sysfs "	\
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	/*
	 * For the purpose of kenobi, waiting for disk IO is an
...
static void do_dbs_timer(struct work_struct *work)
{
    ...
    queue_delayed_work_on(cpu, kkenobi_wq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
    ...
    queue_delayed_work_on(dbs_info->cpu, kkenobi_wq, &dbs_info->work,
			  delay);
}
				...
				unsigned int event)
{
	...
	kenobi_powersave_bias_init_cpu(cpu);
	/*
...
static int __init cpufreq_gov_dbs_init(void)
{
    ...
    kkenobi_wq = create_workqueue("kkenobi");
    if (!kkenobi_wq) {
	printk(KERN_ERR "Creation of kkenobi failed\n");
	return -EFAULT;
    }

    idle_notifier_register(&idle_notifier_block);
    err = cpufreq_register_governor(&cpufreq_gov_kenobi);
    if (err)
	destroy_workqueue(kkenobi_wq);

#ifdef CONFIG_HAS_EARLYSUSPEND
    register_early_suspend(&kenobi_suspend);
#endif
...
static void __exit cpufreq_gov_dbs_exit(void)
{
    ...
    cpufreq_unregister_governor(&cpufreq_gov_kenobi);
    destroy_workqueue(kkenobi_wq);
}
...
MODULE_AUTHOR("Sar Castillo <sar.castillo@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_kenobi' - A dynamic cpufreq governor for "
		   "Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_KENOBI
